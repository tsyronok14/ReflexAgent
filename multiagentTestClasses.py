...
def parseTreeProblem(testDict):
    ...
    return MultiagentTreeProblem(
        numAgents, startState, winStates, loseStates, successors, evaluation
    )
...
class GradingAgent(Agent):
    ...
    def getAction(self, state):
        ...
        studentAction = (
            self.studentAgent.getAction(state),
...
class PolyAgent(Agent):
    ...
    def getAction(self, state):
        ...
        partialPlyBugLists = [
            agent.getBestPacmanActions(state)[0]
            for agent in self.partialPlyBugAgents
        ]
        # record responses
...
class PacmanGameTreeTest(testClasses.TestCase):
    ...
    def execute(self, grades, moduleDict, solutionDict):
        ...
        allActions = [
            json.loads(x) for x in solutionDict["optimalActions"].split("\n")
        ]
        altDepthActions = [
        ...
        partialPlyBugActions = [
            json.loads(x)
            for x in solutionDict["partialPlyBugActions"].split("\n")
        ]
...
class EvalAgentTest(testClasses.TestCase):
    ...
    def execute(self, grades, moduleDict, solutionDict):
        ...
        agentOpts = (
            pacman.parseAgentArgs(self.agentArgs)
            if self.agentArgs != ""
            else {}
        )
